...
fn test_pipeline_simple_struct() {
    ...
    assert!(
        ...
        "id must be non-optional:\n{code}"
    );
    assert!(
        code.contains("pub email: String,"),
        "email must be non-optional:\n{code}"
    );
    assert!(
        code.contains("pub age: Option<i64>,"),
...
fn test_pipeline_allof_composition() {
    ...
    assert!(
        code.contains("pub struct Extended {"),
        "Expected Extended struct:\n{code}"
    );
    assert!(
        code.contains("pub id: String,"),
        "id should be non-optional (required in Base):\n{code}"
    );
    assert!(
        code.contains("pub extra: Option<String>,"),
...
fn test_pipeline_oneof_union() {
    ...
    assert!(
        code.contains("pub enum Pet {"),
        "Expected Pet enum:\n{code}"
    );
    assert!(
        code.contains("#[serde(untagged)]"),
        "oneOf must be untagged:\n{code}"
    );
    assert!(
        code.contains("/// Pet (oneOf)"),
        "Expected oneOf doc comment:\n{code}"
    );
}
...
fn test_pipeline_anyof_union() {
    ...
    assert!(
        code.contains("pub enum Alert {"),
        "Expected Alert enum:\n{code}"
    );
    assert!(
        code.contains("/// Alert (anyOf)"),
        "Expected anyOf doc comment:\n{code}"
    );
}
...
fn test_pipeline_type_alias_from_x_rust_type() {
    ...
    assert!(
        code.contains("pub type UserId = uuid::Uuid;"),
        "Expected type alias:\n{code}"
    );
}
...
fn test_pipeline_requests_and_responses_with_all_mode() {
    ...
    assert!(
        code.contains("pub struct CreateUserRequest {"),
        "Expected request struct:\n{code}"
    );
    assert!(
        code.contains("pub struct CreateUserResponse201 {"),
...
fn test_pipeline_chrono_import_added_for_datetime_field() {
    ...
    assert!(
        code.contains("DateTime"),
        "Expected DateTime in chrono import:\n{code}"
    );
    assert!(
        code.contains("pub occurred_at: DateTime<Utc>,"),
        "Expected DateTime<Utc> field:\n{code}"
    );
}
...
fn test_pipeline_date_import_added_for_date_field() {
    ...
    assert!(
        code.contains("pub date: NaiveDate,"),
        "Expected NaiveDate field:\n{code}"
    );
}
...
fn test_pipeline_uuid_import_added_for_uuid_field() {
    ...
    assert!(
        code.contains("use uuid::Uuid;"),
        "Expected uuid import:\n{code}"
    );
    assert!(
        code.contains("pub id: Uuid,"),
        "Expected Uuid field:\n{code}"
    );
}
...
fn test_pipeline_inline_nested_object_generates_struct() {
    ...
    assert!(
        code.contains("pub struct Order {"),
        "Expected Order struct:\n{code}"
    );
    assert!(
        code.contains("pub struct Address {"),
        "Expected inline Address struct:\n{code}"
    );
}
...
"#;
    ...
    assert!(
        ...
        "Expected Widget struct from YAML spec:\n{code}"
    );
...
fn test_pipeline_empty_spec_produces_no_models() {
    ...
    let code = generate_models(&models, &requests, &responses, GenerateMode::ALL, false)
        .expect("generate_models failed");
    assert!(
        code.contains("use serde::{Serialize, Deserialize};"),
        "Should still have serde import"
    );
}
...
fn test_pipeline_boolean_and_number_fields() {
    ...
    assert!(
        code.contains("pub enabled: bool,"),
        "Expected bool field:\n{code}"
    );
    assert!(
        code.contains("pub ratio: f64,"),
        "Expected f64 field:\n{code}"
    );
    assert!(
